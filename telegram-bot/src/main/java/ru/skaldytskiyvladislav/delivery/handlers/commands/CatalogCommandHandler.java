...
public class CatalogCommandHandler implements CommandHandler, UpdateHandler {
    ...
    @Autowired
    public CatalogCommandHandler(@Lazy CommandHandlerRegistry commandHandlerRegistry, CategoryRepository categoryRepository, ProductRepository productRepository, CartRepository cartRepository, MessageService messageService) {
    ...
    private InlineKeyboardMarkup createProductKeyboard(Long chatId, Product product) {
        ...
        if (cartItem != null) {
            ...
            keyboardBuilder.keyboardRow(Arrays.asList(
                    InlineKeyboardButton.builder()
                        .text(String.format("\uD83D\uDCB5 Цена: %d ₽ \uD83D\uDECD Добавить в корзину", product.getPrice()))
