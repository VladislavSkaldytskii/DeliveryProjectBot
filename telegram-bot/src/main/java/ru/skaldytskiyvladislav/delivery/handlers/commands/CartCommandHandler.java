...
import ru.skaldytskiyvladislav.delivery.models.domain.CartItem;
...
import ru.skaldytskiyvladislav.delivery.models.domain.MessagePlaceholder;
...
public class CartCommandHandler implements CommandHandler, UpdateHandler {
    ...
    @Autowired
    public CartCommandHandler(@Lazy CommandHandlerRegistry commandHandlerRegistry, ClientCommandStateRepository clientCommandStateRepository, ClientOrderStateRepository clientOrderStateRepository, CartRepository cartRepository, ClientRepository clientRepository, MessageService messageService) {
    ...
    private InlineKeyboardMarkup createCartKeyboard(List<CartItem> cartItems, int currentCartPage) {
        ...
        keyboardBuilder.keyboardRow(Arrays.asList(
                InlineKeyboardButton.builder()
                    .text(String.format("\u2705 Оформить %d ₽ заказ?", clientOrder.calculateTotalPrice()))
